...
public class Lexer {
	...
	private void createNextToken() {
		...
		if (state == LexerState.BASIC) {
			...
			if (isOperator()) {
				...
				return;
			} else if (SmartScriptParser.inStringInTag == true && isLetter(data[currentIndex])) {
				wordToken();
				return;
	...
	private void numberToken() {
		...
		if (n % 1 == 0) {
			token = new Token(TokenType.NUMBER, (int) n);
			return;
		}
	...
	private void wordToken() {
		...
		do {
			...
			if (data.length == currentIndex)
				break;
		} while (isLetter(data[currentIndex]) || data[currentIndex] == '\\');
